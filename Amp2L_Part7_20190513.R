...
# factorize variables
clfct <- c("very", "Language", "File", "Adjective", "Function", "Priming", 
...
for(i in 1:7) {
  temp.nsrf <- randomForest(very ~ ., data=nsd, mtry=i, ntree=3000)
...
# set seed
set.seed(201905102)
nsrf <- randomForest(very ~ ., data=nsd, ntree=3000, proximity=TRUE, mtry = 2)
nsrf 
...
png("images/VarImpRF1_RelativeFrequency.png",  width = 680, height = 480) # save plot
...
# we switch to glmer (suggested by R) and also create a lmer object 
# of the final minimal adequate model as some functions
# do not (yet) work on glmer
...
#	manual modelfitting
m0.glmer <- glmer(NonNativeLike ~ 1+ (1|Adjective)+(1|Language), family = binomial, data = nnsd, 
...
anova(m1.glmer, m0.glmer, test = "Chi") # SIG! (p<0.00000000006025 ***)
...
anova(m2.glmer, m1.glmer, test = "Chi") # SIG! (p=0.0000000000001852 ***)
...
anova(m3.glmer, m2.glmer, test = "Chi") # not sig (p=0.1829)
...
anova(m4.glmer, m2.glmer, test = "Chi") # SIG! (p=0.0000000001437 ***)
...
m7.glm <- update(m4.glm, .~.+Emotionality)
ifelse(max(vif(m7.glm)) <= 3,  "vifs ok", "vifs unacceptable") # VIFs ok
m7.glmer <- update(m4.glmer, .~.+Emotionality)
anova(m7.glmer, m4.glmer, test = "Chi") # SIG! (p=0.03258 *)
...
anova(m11.glmer, m7.glmer, test = "Chi") # SIG! (p=0.01548) BUT BIC inflation and minor AIC decrease
...
anova(m12.glmer, m7.glmer, test = "Chi") # not sig! (p=0.1331)
...
anova(m16.glmer, m12.glmer, test = "Chi") # not sig (p=0.6636)
...
anova(m17.glmer, m12.glmer, test = "Chi") # not sig (p=0.1315)
...
write.table(meblrm_nnsd, "meblrm_nnsd_RelativeFrequency.txt", sep="\t")
...
# save results to disc
write.table(meblrm_nnsd_Anova, "meblrm_nnsd_Anova_RelativeFrequency.txt", sep="\t")

effectfunction <- anova(m0.glmer, m1.glmer, test = "Chi")

effectpriming <- anova(m2.glmer, m1.glmer, test = "Chi")

effectfrequency <- anova(m4.glmer, m2.glmer, test = "Chi")

effectemotionality <- anova(m4.glmer, m7.glmer, test = "Chi")

effectfunction_frequency <- anova(m7.glmer, m12.glmer, test = "Chi")

# use customized model comparison function
# create comparisons
m1.m0 <- anova(m1.glmer, m0.glmer, test = "Chi") # SIG! (p<0.00000008724 ***)
m2.m1 <- anova(m2.glmer, m1.glmer, test = "Chi") # SIG! (p=0.0000000007892 ***)
m3.m2 <- anova(m3.glmer, m2.glmer, test = "Chi") # not sig (p=0.5619)
m4.m2 <- anova(m4.glmer, m2.glmer, test = "Chi") # SIG! (p=0.000007447 ***)
m5.m4 <- anova(m5.glmer, m4.glmer, test = "Chi") # not sig (p=0.7936)
m7.m4 <- anova(m7.glmer, m4.glmer, test = "Chi") # SIG! (p=0.02741 *)
m11.m7 <- anova(m11.glmer, m7.glmer, test = "Chi") # mar sig! (p=0.09429) BUT BIC inflation and minor AIC decrease
m12.m7 <- anova(m12.glmer, m7.glmer, test = "Chi") # SIG! (p=0.02092 *)
m16.m12 <- anova(m16.glmer, m12.glmer, test = "Chi") # not sig (p=0.7588)
m17.m12 <- anova(m17.glmer, m12.glmer, test = "Chi") # not sig (p=0.1315)
# create a list of the model compariNonNativeLikens
mdlcmp <- list(m1.m0, m2.m1, m3.m2, m4.m2, m5.m4, m7.m4, m11.m7, m12.m7, m16.m12, m17.m12)
# load function
...
write.table(mdl.cmp.glmersc.swsu.dm, "mdl_cmp_glmersc_swsu_nnsd_RelativeFrequency.txt", sep="\t")
################################################################
#                 IMPORTANT OBJECTS
################################################################
# inspect NonNativeLike important objects
head(nnsd)
...
ggsave(file = paste(imageDirectory,"PredFunction_RelativeFrequency.png",sep="/"), 
...
p2 <- ggplot(pd, aes(Priming, Prediction)) +
  ...
  scale_color_manual(values = c("grey30", "grey30"))
ggsave(file = paste(imageDirectory,"PredPriming_RelativeFrequency.png",sep="/"), 
       height = 5,  width = 3, dpi = 320)
# activate (remove #) to show
p2
...
p3 <- ggplot(pd, aes(x = Frequency, y = Prediction)) +
  ...
  guides(size = FALSE)+
  guides(alpha = FALSE)
ggsave(file = paste(imageDirectory,"PredFrequency_RelativeFrequency.png",sep="/"), 
       height = 5,  width = 3, dpi = 320)
p3
...
p4 <- ggplot(pd, aes(Emotionality, Prediction)) +
  ...
  guides(size = FALSE)+
  guides(alpha = FALSE)
ggsave(file = paste(imageDirectory,"PredEmotionality_RelativeFrequency.png",sep="/"), 
...
ggsave(file = paste(imageDirectory,"PredFrequencyFunction_RelativeFrequency.png",sep="/"), 
...
ggsave(file = paste(imageDirectory,"RanLanguage_RelativeFrequency.png",sep="/"), 
...
ggsave(file = paste(imageDirectory,"RanAdjective_RelativeFrequency.png",sep="/"), 
       height = 5,  width = 5,  dpi = 320)
# activate (remove #) to show
p7
...
p8 <- ggplot(pd, aes(x = Frequency, y = Prediction)) +
  ...
  labs(x = "Frequency of adj. type", y = "Predicted probability of \nnon-native-like choices") +
  scale_color_manual(values = c("grey30",  "grey50"))   
ggsave(file = paste(imageDirectory,"PredRelFrequencyFunction_RelativeFrequency.png",sep="/"), 
       height = 5,  width = 5,  dpi = 320)
p8
